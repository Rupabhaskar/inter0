...
export default function ExamTestPage({ params }) {
  ...
  useEffect(() => {
    const onFull = () => {
      ...
      if (!active && isFullscreen && started && !submitted) {
        ...
        if (warningDialogOpenRef.current) return;
        if (Date.now() < suppressViolationUntilRef.current) return;
        const now = Date.now();
        if (now - lastViolationAtRef.current < 800) return;
        lastViolationAtRef.current = now;
        setPauseReason("You exited fullscreen mode.");
        setIsPaused(true);
        setSecurityWarnings((prev) => {
          const next = prev + 1;
          if (next >= 3) {
            securityTriggeredRef.current = true;
            setPauseReason("Warning limit reached (3/3). Submitting exam.");
            setIsPaused(false);
            submitTest();
            return next;
          }
          warningDialogOpenRef.current = true;
          setWarningDialog({
            open: true,
            message: `Warning ${next}/3: You exited fullscreen mode.`,
          });
          return next;
        });
      }
  ...
  useEffect(() => {
    const registerViolation = (reason) => {
      if (!started || submitted || securityTriggeredRef.current) return;
      if (warningDialogOpenRef.current) return;
      if (Date.now() < suppressViolationUntilRef.current) return;
      const now = Date.now();
      if (now - lastViolationAtRef.current < 800) return;
      ...
      setSecurityWarnings((prev) => {
        ...
        warningDialogOpenRef.current = true;
        setWarningDialog({ open: true, message: `Warning ${next}/3: ${reason}` });
        return next;
      });
    };
    ...
    const block = (e) => {
      ...
      const blockedCtrlKeys = ["t", "w", "n", "r", "l", "p", "s", "u", "c", "v", "x", "a", "j", "i"];
      const blocked =
        key === "tab" ||
        key === "alt" ||
        key === "control" ||
        ...
        e.altKey ||
        e.ctrlKey ||
        (e.ctrlKey && blockedCtrlKeys.includes(key)) ||
        e.metaKey;
      if (blocked) {
        e.preventDefault();
        e.stopPropagation();
        registerViolation("Restricted keyboard key/shortcut detected.");
      }
    };

    const onKeyUp = (e) => {
      const key = String(e.key || "").toLowerCase();
      if (key === "alt" || key === "control" || key === "meta" || key === "tab") {
        registerViolation("Restricted keyboard key detected.");
      }
    };
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- submitTest is intentionally invoked on security events
  }, [started, submitted]);
  ...
  useEffect(() => {
    ...
    const onPageHide = () => {
      registerViolation("Page hide detected.");
    };
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps -- submitTest is intentionally invoked on focus loss
  }, [started, submitted]);

  const startTest = async () => {
    try {
      securityTriggeredRef.current = false;
      lastViolationAtRef.current = 0;
      warningDialogOpenRef.current = false;
      suppressViolationUntilRef.current = 0;
  ...
  const handleWarningContinue = () => {
    warningDialogOpenRef.current = false;
    suppressViolationUntilRef.current = Date.now() + 1500;
    setWarningDialog({ open: false, message: "" });
    setPauseReason("");
    setIsPaused(false);
  };

  const handleWarningSubmitNow = () => {
    warningDialogOpenRef.current = false;
    securityTriggeredRef.current = true;
    setWarningDialog({ open: false, message: "" });
    setPauseReason("Submitted by user from warning dialog.");
    setIsPaused(false);
    submitTest();
  };
        ...
        <div className="min-h-screen flex items-center justify-center bg-gray-100">
          <div className="bg-white p-8 rounded shadow text-center max-w-md">
            ...
            <p className="text-gray-600 mb-4">
              {pauseReason || "Security rule triggered."}
            </p>
            <p className="text-sm text-orange-600 mb-3">
              Security warnings: {securityWarnings}/3
            </p>
            <p className="text-gray-600 mb-4">
              Time remaining:{" "}
  ...
  return (
    <ProtectedRoute>
      ...
      {warningDialog.open && !submitted && (
        <div className="fixed inset-0 z-[120] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
            <h3 className="text-lg font-bold text-orange-700 mb-2">Security Warning</h3>
            <p className="text-slate-700 mb-3">{warningDialog.message}</p>
            <p className="text-sm text-slate-500 mb-5">
              You can continue the exam, or submit now.
            </p>
            <div className="flex justify-end gap-2">
              ...
              <button
                type="button"
                onClick={handleWarningSubmitNow}
